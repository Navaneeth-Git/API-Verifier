...
async function verifyWithProvider(provider: Provider, key: string) {
  ...
  const response = await requests[provider](); if (response.ok) return { status: 'valid', message: 'Authentication succeeded.' }; if (response.status === 401 || response.status === 403) return { status: 'invalid', message: 'The provider rejected this credential. Check that it is complete and active.' }; return { status: 'error', message: `The provider returned an unexpected response (${response.status}).` }
}
